...
export function analyzeSeo(html, url, responseTime) {
  ...
  const descriptionResult = analyzeDescription($);
  const ogResult = analyzeOGTags($, url);
  const twitterResult = analyzeTwitterTags($);
  ...
  const analysis = {
    ...
    robots: analyzeRobots($),
    h1: analyzeH1($),
    lang: analyzeLang($),
    favicon: analyzeFavicon($),
    structuredData: analyzeStructuredData($),
    performance: analyzePerformance($, html, responseTime),
...
function analyzePerformance($, html, responseTime) {
  ...
  const images = $('img');
  const imagesWithAlt = images.filter((_, el) => $(el).attr('alt') !== undefined);
  const imagesWithoutAlt = images.filter((_, el) => $(el).attr('alt') === undefined);
  const asyncScripts = $('script[src][async]');
  const deferScripts = $('script[src][defer]');
  ...
  const blockingLinks = $('link[rel="stylesheet"]').length;
...
function getMetaByName($, name) {
  const el = $(`meta[name="${name}"], meta[name="${name.toLowerCase()}"]`).first();
...
function analyzeTitle($) {
  const content = $('title').first().text().trim();
...
function analyzeDescription($) {
  const content = getMetaByName($, 'description');
  const length = content.length;
  const issues = [];
  const recommendations = [];
...
function analyzeOGTags($, url) {
  const tags = {
    title: { content: getMetaByProperty($, 'og:title') },
    description: { content: getMetaByProperty($, 'og:description') },
    image: { content: getMetaByProperty($, 'og:image') },
    url: { content: getMetaByProperty($, 'og:url') },
    type: { content: getMetaByProperty($, 'og:type') },
    siteName: { content: getMetaByProperty($, 'og:site_name') },
  };
  const issues = [];
  const recommendations = [];
...
function analyzeTwitterTags($) {
  const tags = {
    card: { content: getMetaByName($, 'twitter:card') },
    site: { content: getMetaByName($, 'twitter:site') },
    title: { content: getMetaByName($, 'twitter:title') },
    description: { content: getMetaByName($, 'twitter:description') },
    image: { content: getMetaByName($, 'twitter:image') },
  };
  const issues = [];
  const recommendations = [];
...
function analyzeCanonical($, url) {
  ...
  const content = el.attr('href')?.trim() || '';
  const issues = [];
  const recommendations = [];
...
function analyzeViewport($) {
  const content = getMetaByName($, 'viewport');
...
function analyzeCharset($) {
  const meta1 = $('meta[charset]').first();
  const meta2 = $('meta[http-equiv="Content-Type"]').first();

  const content = meta1.attr('charset') || meta2.attr('content') || '';
...
function analyzeRobots($) {
  const content = getMetaByName($, 'robots');
  const issues = [];
  const recommendations = [];
...
function analyzeH1($) {
  ...
  const content = h1s.first().text().trim();
  const issues = [];
  const recommendations = [];
...
function analyzeFavicon($) {
  ...
  const favicons = links.toArray().map(el => ({ rel: $(el).attr('rel'), href: $(el).attr('href') }));
